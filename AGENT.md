...
- phytozome GO FASTA headers use the current backslash-delimited `phgo://` schema. Keyword records use `phgo://species_short/label_name/id2\row` when a row number exists. BLAST hit records use `phgo://species_short/hit_label_name/hit_id2\blast_source_label_name/blast_source_id2\row`. Prepended BLAST query/source records also use phgo headers when that export option is enabled, with `phgo://species_short/source_label_name/source_id2\h`. BLAST input parsing must recognize these current forms, including species or labels containing spaces, and must treat the primary `label_name/id2` as the pasted query sequence identity. Old slash-row phgo variants do not need compatibility work, but accidental generic FASTA parsing does not need to be blocked.
- Family BLAST FASTA export defaults to prepending all family-member BLAST query/source records; the "only first query sequence" option is available but must be unchecked by default.
