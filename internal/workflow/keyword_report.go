...
func keywordProvenance(rows []model.KeywordResultRow) []report.ProvenanceSlice {
	...
	for _, row := range rows {
		...
		for _, value := range keywordReportSourceValues(row) {
			...
			case "phytozome":
				if rowUsesKeywordWideFallback(row) {
					counts["fallback already performed"]++
				} else {
	...
	labels := []string{"direct source result", "local release parsed", "fallback already performed", "generated/internal", "unavailable/missing"}
...
func rowUsesKeywordWideFallback(row model.KeywordResultRow) bool {
	return strings.Contains(strings.ToLower(row.SearchType), "fallback to wide search")
}
...
func provenanceExplanation(label string) string {
	...
	case "fallback already performed":
		return "Values present because the new keyword search engine fell back to wide search during the actual search run."
...
func keywordQualityChecks(groups []model.KeywordSearchGroup, rows []model.KeywordResultRow, allRows []model.KeywordResultRow, settings exportSettings, sequenceAudit report.SequenceAudit) []report.QualityCheck {
	...
	checks := []report.QualityCheck{
		...
		qualityCheck("Generated table rows missing search type", countKeywordRowsWhere(tableRows, func(row model.KeywordResultRow) bool { return strings.TrimSpace(row.SearchType) == "" }) == 0, fmt.Sprintf("%d of %d", countKeywordRowsWhere(tableRows, func(row model.KeywordResultRow) bool { return strings.TrimSpace(row.SearchType) == "" }), totalRows), "warn when any exported table row lacks search_type", "Search type records which keyword search program produced the row and whether wide-search fallback was used.", "generated keyword table"),
