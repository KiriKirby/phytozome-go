...
func RunDisk(ctx context.Context, fn func(context.Context) error) error {
	return RunTaskSpec(ctx, TaskSpec{Level: ExecHeavy, LocalSlots: 1, Description: "disk task"}, fn)
}
...
func resourceRequestForTaskSpec(spec TaskSpec) ResourceRequest {
	...
	if request.LocalSlots <= 0 && spec.Level != ExecInline {
...
func missingResourceRequestForTaskSpec(ctx context.Context, spec TaskSpec) ResourceRequest {
	...
	if request.LocalSlots > 0 {
		if grant, ok := contextLocalGrant(ctx); ok && grant != nil {
			if request.LocalLevel == grant.Level && grant.Slots >= request.LocalSlots {
...
func resourceRequestIsEmpty(request ResourceRequest) bool {
	return request.LocalSlots <= 0 && len(request.Network) == 0
}
