...
func (t *domainAwareTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	grant, release, err := AcquireNetwork(contextForRequest(req), domain, 1)
	if err != nil {
		return nil, err
	}
	_ = grant
	...
	if err != nil {
		release(time.Since(started), err, false)
		return nil, err
	}
	rateLimited := resp.StatusCode == http.StatusTooManyRequests
	serverErr := resp.StatusCode >= 500
	if resp.Body == nil {
		release(time.Since(started), statusError(resp, err), rateLimited || serverErr)
		return resp, nil
	}
